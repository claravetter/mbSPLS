...
| `matrix_norm`                                       | String/Double   | Define which matrix norm is used to compute the association between latent scores of Xs (used for significance testing of LV). [If > 2 Xs:] `fro` (Default; [If 2 Xs:]  `0` (Correlation coefficient as defined above is used). |
